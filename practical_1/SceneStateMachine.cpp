...
void SceneStateMachine::Remove(unsigned int id) {
	...
	if (it != scenes.end()) {
		if (curScene == it->second) {
			//If the scene we are removing is the current scene, we also want to set that to a null pointer
...
void SceneStateMachine::SwitchTo(unsigned int id) {
	...
	if (it != scenes.end()) {
		...
		//Setting the current scene ensures that it is updated & drawn
		curScene = it->second;
